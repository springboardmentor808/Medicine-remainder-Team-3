"""
PillSync WHO Essential Medicines Dosage Benchmarks (Track 3 -- Engineer 3).
...
WHO EML Source: https://list.essentialmeds.org/

Usage:
    from ai_training.src.who_dosage_benchmarks import WHODosageBenchmarks

    benchmarks = WHODosageBenchmarks()
    limit = benchmarks.get_max_daily_dose("Metformin")
    # -> {"salt": "Metformin", "max_daily_mg": 2550, "adult_dose_range": "500-2550mg",
    #     "frequency": "2-3 times daily", "pregnancy_category": "B"}
"""
...
# ---------------------------------------------------------------------------
# WHO / Pharmacopoeia Maximum Daily Dosage Reference Table
# Sourced from: WHO EML 23rd list, BNF, CIMS India, MIMS India
#
# Format: salt_name -> {
#   max_daily_mg: maximum safe daily dose in mg for adults,
#   adult_dose_range: typical adult dosage range string,
#   pediatric_dose_mg_per_kg: max mg/kg/day for children (if applicable),
#   frequency: typical dosing frequency,
#   pregnancy_category: FDA/WHO pregnancy risk category (A/B/C/D/X),
#   renal_adjustment: whether dose needs adjustment in renal impairment,
#   hepatic_adjustment: whether dose needs adjustment in hepatic impairment,
...
class WHODosageBenchmarks:
    ...
    def get_max_daily_dose(self, salt_name: str) -> Optional[dict]:
        """
        Look up the maximum safe daily dose for a given salt.

        Returns:
            dict with dosage limit info, or None if not found
        """
    ...
    def validate_daily_dose(
        ...
        """
        Validate whether a proposed daily dose exceeds safe limits.

        Args:
            salt_name: Active ingredient name
            total_daily_mg: Total mg the patient would take per day

        Returns:
            dict with validation result, severity, and recommendation
        """
    ...
    def get_pregnancy_category(self, salt_name: str) -> Optional[str]:
        """Get the pregnancy safety category (A/B/C/D/X) for a drug."""
        limit = self.get_max_daily_dose(salt_name)
        return limit["pregnancy_category"] if limit else None

    def get_all_benchmarks(self) -> dict:
        """Return the full dosage limits dictionary."""
...
if __name__ == "__main__":
    print("=" * 60)
    print("  PillSync WHO Dosage Benchmarks Engine")
    print("=" * 60)

    benchmarks = WHODosageBenchmarks()

    print(f"\n  Total benchmarked drugs: {len(DOSAGE_LIMITS)}")

    # Test lookups
    test_cases = [
        ("Paracetamol", 2000),   # Safe
        ("Paracetamol", 4500),   # Danger
        ("Metformin", 1500),     # Safe
        ("Metformin", 3000),     # Danger
        ("Amlodipine", 10),      # Safe
        ("Amlodipine", 15),      # Danger
        ("Azithromycin", 500),   # Safe
    ]

    print(f"\n--- Dosage Validation Tests ---")
    for salt, dose in test_cases:
        result = benchmarks.validate_daily_dose(salt, dose)
        status = result["status"]
        icon = {"SAFE": "[OK]", "WARNING": "[WARN]", "DANGER": "[DANGER]", "UNKNOWN": "[?]"}.get(status, "")
        print(f"  {icon:10s} {salt:20s} {dose:>7.1f}mg/day -> {result['message']}")

    # Pregnancy categories
    print(f"\n--- Pregnancy Safety Categories ---")
    preg_drugs = ["Paracetamol", "Metformin", "Atorvastatin", "Telmisartan", "Folic Acid"]
    for drug in preg_drugs:
        cat = benchmarks.get_pregnancy_category(drug)
        print(f"  {drug:20s} -> Category {cat}")

    print(f"\n  [OK] WHO Dosage Benchmarks Engine ready")
